...
class EventServer extends Logging with Serializable {
  ...
  def getEventCache(eventCacheService:ExecutorCompletionService[immutable.Map[String, (String, Array[Byte])]],
    ...
    // 满足周期输出的key 和json 。outPutJsogetEventCachenMap :Map[key->json]
    val outPutJsonMap = Map[String, String]()
    batchList.foreach(batch => eventCacheService.submit(new QryEventCache(batch, eventId)))
    //增加cacheEventTimeMap,用来存放最新cache time
   // val cacheEventTimeMap = mutable.Map[String, String]()
    // 遍历各batch线程的结果返回值
    for (index <- 0 until batchList.size) {
      ...
      if (result != null && result.size > 0) {
        result.foreach(rs => {
          ...
          //从cacheEventTimeMap中获得最新的营销时间
        //  val eventTime = cacheEventTimeMap.getOrElse(key, cache_time)
          val current_time = System.currentTimeMillis
          // 满足营销
          if (current_time >= (cache_time.toLong + interval.toLong * 1000L)) {
            //更新cacheEventTimeMap
            //cacheEventTimeMap += (key -> String.valueOf(current_time))
            // 放入更新codis list等待更新
