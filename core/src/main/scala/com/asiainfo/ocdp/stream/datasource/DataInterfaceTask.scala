  ...
  def execLabels(df: DataFrame): RDD[String] = {
    df.toJSON.mapPartitions(iter => {
      ...
      iter.toList.map(jsonStr => {
        ...
        labels.foreach(label => {
          val qryKeys = label.getQryKeys(currentLine)
          if (qryKeys != null && qryKeys.nonEmpty) labelQryKeysSet ++= qryKeys//获取每个label的查询key，以便批量查询
        })
      })
      // cachemap_old
      val f1 = System.currentTimeMillis()
	 ...
	 println("cache map size:" + cachemap_old.size)
      ...
      val f2 = System.currentTimeMillis()
      println(" 1. 一个partition，根据uk去codis查询Lable:$uk数据耗时: " + (f2 - f1) + " millis ! ")
      //2.查询所有的label标签数据：【userinfo:$UK】,【lacci2area:$lac:$ci】,
      val labelQryData = CacheFactory.getManager.hgetall(labelQryKeysSet.toList, hgetAllService)
      val f3 = System.currentTimeMillis()
      println(" 2. 一个partition，根据uk去codis查询标签数据【userinfo:$UK】,【lacci2area:$lac:$ci】等数据耗时: " + (f3 - f2) + " millis ! ")
      //3.根据1和2的查询结果，遍历整个批次的数据，逐条记录打标签
     //累加器
      val lableAccuMap = mutable.Map[String, Long]()
      labels.foreach(label => {
        lableAccuMap+= (label.conf.getId -> 0L)
      })
      val jsonList = busnessKeyList.map(enum => {
        ...
        labels.foreach(label => {
          val if1 = System.currentTimeMillis()
          ...
          val resultTuple = label.attachLabel(value, old_cache, labelQryData)
          val if2 = System.currentTimeMillis()
          //println(label.conf.getName +" attachLable: " + (if2 - if1) + " millis ! ")
          var accum=(lableAccuMap.get(label.conf.getId) match {
            case Some(accum) =>accum.asInstanceOf[Long]
            case None => 0L
          })
          accum=accum+(if2 - if1)
          lableAccuMap+= (label.conf.getId -> accum)
      ...
      val f4 = System.currentTimeMillis()
      println(" 3. 根据（2）和（3）的查询结果，进行lable.attachLabel操作总耗时: " + (f4 - f3) + " millis ! ")
      println("labelCostCompare=")
      lableAccuMap.map(accum=>println(accum._1+":"+accum._2))

      //4.update caches to CacheManager
      CacheFactory.getManager.setMultiCache(cachemap_new)
      println(" 4. 更新这批数据的lable缓存Lable:$uk耗时: " + (System.currentTimeMillis() - f4) + " millis ! ")
      jsonList.iterator
    })
  }
